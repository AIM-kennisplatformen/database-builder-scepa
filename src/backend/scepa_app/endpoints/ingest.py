...
def _run_ingestion() -> IngestResponse:
    ...
    from scepa_app.main import store_graph, store_vectors, connect_qdrant, connect_typedb
    ...
    from database_builder_libs.utility.extract.document_parser_docling import DocumentParserDocling

    # Get all pending documents from the JSON registry
    registry = _load_registry()
    eligible = [(h, e) for h, e in registry.items() if e["status"] in ("pending", "approved")]
    if not eligible:
    ...
    embedder = OpenAICompatibleChunkEmbedder(base_url=settings.openai_host, api_key=settings.openai_key, model=settings.embedding_model)
    results: list[IngestResult] = []

    for doc_hash, entry in eligible:
        ...
        try:
            ...
            chunks = SummaryAndSectionsStrategy().chunk(doc.sections, document_id=doc_hash)
            chunks = embedder.embed(chunks)
            for c in chunks:
                c.metadata = {**(c.metadata or {}), "document_hash": doc_hash}

            # Store vectors in Qdrant and graph nodes in TypeDB
            store_vectors(chunks, qdrant)
            content = Content(date=datetime.fromtimestamp(file_path.stat().st_mtime), id_=doc_hash, content={"metadata": entry["metadata"]})
            nodes = store_graph(content, typedb, doc_hash=doc_hash)

            entry["status"] = QueueStatus.INGESTED.value
            _save_registry(registry)
            results.append(IngestResult(document_hash=doc_hash, filename=entry["filename"], status="ingested", message=f"{len(chunks)} chunks, {len(nodes)} nodes"))
        except Exception as e:
            logger.exception("Ingestion failed for %s", doc_hash)
            entry["status"] = QueueStatus.FAILED.value
            _save_registry(registry)
            results.append(IngestResult(document_hash=doc_hash, filename=entry["filename"], status="failed", message=f"{type(e).__name__}: {str(e)[:200]}"))

    return IngestResponse(total=len(results), ingested=sum(1 for r in results if r.status == "ingested"), failed=sum(1 for r in results if r.status == "failed"), results=results)
